            ...
            {result.producer && result.producerRow ? (
              <div className="flex gap-4 items-start text-sm">
                {getFactoryImage(result.producerRow.token) && (
                  <img 
                    src={getFactoryImage(result.producerRow.token)} 
                    alt={result.producerRow.token} 
                    className="h-16 w-16 shrink-0 bg-slate-900/60 object-contain p-1" 
                    style={{ borderRadius: 'var(--radius-resource-item)' }}
                  />
                )}
                <div className="space-y-2">
                  <p className="text-lg font-semibold">{formatPlotName(result.producer.landPlotName || (language === 'es' ? 'Parcela desconocida' : 'Unknown plot'), language)} • {formatFactoryName(result.producerRow.token, language)}</p>
                  <p className="text-slate-300">
                    <span className="text-slate-400">{language === 'es' ? 'Nivel de producción actual' : 'Current producing level'}:</span>{' '}
                    <strong>{result.producerRow.level}</strong>
                  </p>
                  <p className="text-slate-300">
                    <span className="text-slate-400">{language === 'es' ? 'Tiempo de Ciclo' : 'Cycle Runtime'}:</span>{' '}
                    <strong>{runtimeText}</strong>
                  </p>
                  <p className="flex items-center gap-1.5 text-slate-300">
                    {getResourceImage(result.selectedToken) && <img src={getResourceImage(result.selectedToken)} alt={result.selectedToken} className="h-4.5 w-4.5 object-contain" />}
                    <span>{fmt(result.outputPerHour)} <strong>{formatFactoryName(result.selectedToken, language)}</strong>/hr</span>
                  </p>
                  <p className="flex items-center gap-1.5 text-slate-300">
                    {getResourceImage(result.selectedToken) && <img src={getResourceImage(result.selectedToken)} alt={result.selectedToken} className="h-4.5 w-4.5 object-contain" />}
                    <span>{fmt(result.outputPerDay)} <strong>{formatFactoryName(result.selectedToken, language)}</strong>/day</span>
                  </p>
                </div>
              </div>
            ) : (
              ...
              <div className="space-y-4 text-sm">
                <div className="flex gap-3 items-center">
                  {getFactoryImage(result.selectedRow.token) && (
                    <img 
                      src={getFactoryImage(result.selectedRow.token)} 
                      alt={result.selectedRow.token} 
                      className="h-12 w-12 shrink-0 bg-slate-900/60 object-contain p-1" 
                      style={{ borderRadius: 'var(--radius-resource-item)' }}
                    />
                  )}
                  <div>
                    <p className="text-lg font-semibold">{formatFactoryName(result.selectedRow.token, language)} • Level {result.selectedRow.level}</p>
                    <p className="flex items-center gap-1.5 text-xs text-slate-400">
                      <span>{language === 'es' ? 'Salida' : 'Output'}:</span>{' '}
                      <strong>{fmt(result.selectedRow.output_amount)}</strong>
                      {getResourceImage(result.selectedRow.output_token) && (
                        <img src={getResourceImage(result.selectedRow.output_token)} alt={result.selectedRow.output_token} className="h-4.5 w-4.5 object-contain inline-block shrink-0" />
                      )}
                      <strong>{formatFactoryName(result.selectedRow.output_token, language)}</strong>
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-1.5 border-t border-slate-800 pt-2">
                  <p className="text-slate-300">
                    <span className="text-slate-400">{language === 'es' ? 'Tiempo de ejecución base' : 'Base runtime'}:</span>{' '}
                    <strong>{formatDurationFromMinutes(result.selectedRow.duration_min)}</strong>
                  </p>
                  <p className="flex items-center gap-1.5 text-slate-300">
                    <span className="text-slate-400">{language === 'es' ? 'Ingrediente 1' : 'Input'}:</span>
                    {getResourceImage(result.selectedRow.input_token_1) && <img src={getResourceImage(result.selectedRow.input_token_1)} alt={result.selectedRow.input_token_1} className="h-4.5 w-4.5 object-contain" />}
                    <span>{fmt(result.selectedRow.input_amount_1)} <strong>{formatFactoryName(result.selectedRow.input_token_1, language)}</strong></span>
                  </p>
                  {result.selectedRow.input_token_2 && result.selectedRow.input_amount_2 > 0 && (
                    <p className="flex items-center gap-1.5 text-slate-300">
                      ...
                      {getResourceImage(result.selectedRow.input_token_2) && <img src={getResourceImage(result.selectedRow.input_token_2)} alt={result.selectedRow.input_token_2} className="h-4.5 w-4.5 object-contain" />}
                      <span>{fmt(result.selectedRow.input_amount_2)} <strong>{formatFactoryName(result.selectedRow.input_token_2, language)}</strong></span>
                    </p>
                  )}
                  <p className="flex items-center gap-1.5 text-slate-300">
                    <span className="text-slate-400">{language === 'es' ? 'Próxima mejora' : 'Next upgrade'}:</span>
                    {getResourceImage(result.selectedRow.upgrade_token) && <img src={getResourceImage(result.selectedRow.upgrade_token)} alt={result.selectedRow.upgrade_token} className="h-4.5 w-4.5 object-contain" />}
                    <span>{fmt(result.selectedRow.upgrade_amount)} <strong>{formatFactoryName(result.selectedRow.upgrade_token, language)}</strong></span>
                  </p>
                </div>
