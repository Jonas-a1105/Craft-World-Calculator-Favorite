...
function nearestSnapshot(history: PriceSnapshot[], symbol: string, targetMs: number) {
  const candidates = history.filter(
    (snapshot) => snapshot.symbol === symbol && getComparablePrice(snapshot) > 0,
  );
  if (!candidates.length) return null;
  return candidates.reduce<PriceSnapshot | null>((best, snapshot) => {
    if (!best) return snapshot;
    return Math.abs(toMs(snapshot.timestamp) - targetMs) < Math.abs(toMs(best.timestamp) - targetMs)
      ? snapshot
      : best;
  }, null);
}
...
export function computePriceDelta(
  ...
  const twentyFourHour = nearestSnapshot(
    history,
    normalized,
    toMs(timestamp) - 24 * 60 * 60 * 1000,
  );
  const oneHourPercent = oneHour ? percentChange(currentPrice, getComparablePrice(oneHour)) : null;
  const twentyFourHourPercent = twentyFourHour
    ? percentChange(currentPrice, getComparablePrice(twentyFourHour))
    : null;
  const signal = twentyFourHourPercent ?? oneHourPercent;
  const state =
    signal === null ? 'missing' : signal > 0.1 ? 'up' : signal < -0.1 ? 'down' : 'neutral';
