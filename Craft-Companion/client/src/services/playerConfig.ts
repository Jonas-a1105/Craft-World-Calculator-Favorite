...
export function normalizeFactoryConfig(
  value: Partial<PlayerFactoryConfig> | undefined,
): PlayerFactoryConfig {
  return {
    enabled: Boolean(value?.enabled),
    factoryCount: Math.max(
      1,
      Math.floor(finiteNumber(value?.factoryCount, DEFAULT_FACTORY_CONFIG.factoryCount)),
    ),
    factoryLevel: Math.max(
      1,
      Math.floor(finiteNumber(value?.factoryLevel, DEFAULT_FACTORY_CONFIG.factoryLevel)),
    ),
    individualLevels: Array.isArray(value?.individualLevels)
      ? value.individualLevels
          .map((level) => Math.max(1, Math.floor(finiteNumber(level, 1))))
          .slice(0, 100)
      : [],
...
export function loadPlayerConfig(storage: Storage = window.localStorage): PlayerConfig {
  try {
    ...
    const factories = Object.entries(parsed.factories || {}).reduce<
      Record<string, PlayerFactoryConfig>
    >((acc, [key, value]) => {
...
export function savePlayerConfig(config: PlayerConfig, storage: Storage = window.localStorage) {
  const normalized: PlayerConfig = {
    version: 1,
    factories: Object.entries(config.factories || {}).reduce<Record<string, PlayerFactoryConfig>>(
      (acc, [key, value]) => {
        acc[key.toUpperCase()] = normalizeFactoryConfig(value);
        return acc;
      },
